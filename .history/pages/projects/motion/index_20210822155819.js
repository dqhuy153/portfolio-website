...
export default function MotionPage(props) {
    ...
    useEffect(() => {
        const preEndAnimationTrigger = () => {
            if (
                window.scrollY + preEndRef.current.clientHeight ===
                preEndRef.current.offsetTop
            ) {
                pPreEndApi.start();
            }
        };

        window.addEventListener('scroll', preEndAnimationTrigger);

        return () => {
            window.removeEventListener('scroll', preEndAnimationTrigger);
        };
    }, [pPreEndApi]);
